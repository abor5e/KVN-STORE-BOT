...
const app = express();
app.get('/', (req, res) => res.send('KVN Bot is running!'));
...
const categoryBanners = {
    'purchase': 'kvn-purchase.png',
    'refund':   'kvn-refund.png',
    'support':  'kvn-support.png'
};
...
client.once('ready', async () => {
    console.log('KVN جاهز: ' + client.user.tag);
    try {
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'ticket') {
            ...
            const mainBannerPaths = [
                path.join(__dirname, 'assets', 'kvn-main.jpg'),
                path.join(process.cwd(), 'assets', 'kvn-main.jpg')
            ];
            const mainBannerPath = mainBannerPaths.find(p => fs.existsSync(p)) || null;

            try {
                if (mainBannerPath) {
                    const attachment = new AttachmentBuilder(mainBannerPath, { name: 'kvn-main.jpg' });
        ...
        if (commandName === 'ticket-stats') {
            ...
            await interaction.reply({
                embeds: [new EmbedBuilder()
                    .setTitle('📊 إحصائيات التذاكر — KVN متجر')
                    .addFields(
        ...
        if (commandName === 'help') {
            await interaction.reply({
                embeds: [new EmbedBuilder().setTitle('📋 أوامر الإدارة — KVN متجر').addFields(
                ...
                ).setColor(0xE8B923).setFooter({ text: 'KVN متجر — أوامر الإدارة' }).setTimestamp()],
                flags: 64
            });
            return;
        }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'ticket-menu') {
        ...
        try {
            ...
            const ticketEmbed = new EmbedBuilder()
                ...
                .setDescription('أهلاً بك في **KVN متجر**! يرجى شرح طلبك وسيتولى أحد الإدارة مساعدتك قريباً.')
                .setColor(0xE8B923)
                .setTimestamp();
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'setup-category-select') {
        ...
        const categoryLabels = { 'purchase': 'شراء', 'refund': 'استرجاع', 'support': 'دعم فني' };
